...
ExternalProgramPaths ExternalExtractorSolver::getDefaultExternalPaths(ComputerSystemType system)
{
    ...
    case LINUX_DEFAULT:
        ...
        (QFile("/bin/astap").exists()) ? paths.astapBinaryPath = "/bin/astap" : paths.astapBinaryPath = "/opt/astap/astap";
...
int ExternalExtractorSolver::runExternalExtractor()
{
    ...
    //------------------------------ Photometry -----------------------------------
    sextractorArgs << "-PHOT_AUTOPARAMS" << QString::number(m_ActiveParameters.kron_fact) + "," + QString::number(m_ActiveParameters.r_min);
...
#else
    emit logOutput("+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++");
    emit logOutput("Not support external SExtractor");
    emit logOutput("+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++");
    return -1;
#endif
}
...
#else
    emit logOutput("+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++");
    emit logOutput("Not support external Astrometry.net solver");
    emit logOutput("+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++");
    return -1;
#endif
}
...
#else
    emit logOutput("+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++");
    emit logOutput("Not support external ASTAP solver");
    emit logOutput("+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++");
    return -1;
#endif
}
...
#else
    emit logOutput("+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++");
    emit logOutput("Not support external Watney Astrometry solver");
    emit logOutput("+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++");
    return -1;
#endif
}
...
QStringList ExternalExtractorSolver::getSolverArgsList()
{
    ...
    if (m_UsePosition)
        solverArgs << "-3" << QString::number(search_ra) << "-4" << QString::number(search_dec) << "-5" << QString::number(m_ActiveParameters.search_radius);
...
bool ExternalExtractorSolver::getWatneySolutionInformation()
{
    ...
    if (!jsonObj.contains("ra") || !jsonObj.contains("dec") || !jsonObj.contains("orientation") || !jsonObj.contains("pixScale") || !jsonObj.contains("parity"))
    {
